...
"James Madison High School": "Site12_East_River_Fifteen",//No data so we connect to East River
...
"BASE High School" :  "Site12_East_River_Fifteen",//No data so we connect to East River
"Mary D. Carter School (PS151Q)": "Site18_MDC_School_Fifteen",
"Montauk IS and High School" : "Site12_East_River_Fifteen",//No data so we connect to East River
"Susan Wagner High School" : "Site12_East_River_Fifteen",//No data so we connect to East River
...
// Border size or look
const borderStyle = '2.5px solid #A9A9A9'
const borderMargin = '0px'

async function fetchData(url) {
try {
  const resp = await fetch(url);
  //https://earth.noaacrest.org/uhmt/data/plot_data_Folder/daily_plot_data/Site1_Queens_Botanical_Garden_Fifteen.json
  ...
  const urlAll = "https://earth.noaacrest.org/uhmt/data/plot_data_Folder/yearly_plot_data/" + filename
  console.log("URLALL",urlAll);

  const allJson = await fetch(urlAll)
  if (!allJson.ok) { throw new Error('HTTP error! Status: ${resp.status}'); }
  const YearlyJson = await allJson.json();
  jsonDataD = YearlyJson;
...
const chart = nv.models.lineChart()
  //.height(230);//Make sure to change .chart height as well
