...
def get_daily():
    ...
    def process_csv_file(file_path):
        try:
            ...
            last_24_hours = df[df['TIMESTAMP'] > datetime.now() - timedelta(days=7)]
            last_24_hours['TIMESTAMP'] = last_24_hours['TIMESTAMP'].dt.strftime('%Y-%m-%d %H:%M:%S')
...
def get_weekly():
    ...
    def process_csv_file(file_path):
        try:
            ...
            df['TIMESTAMP'] = pd.to_datetime(df['TIMESTAMP'])
            last_month = df[df['TIMESTAMP'] > datetime.now() - timedelta(days=30)]
...
def get_monthly():
    ...
    def process_csv_file(file_path):
        try:
            ...
            df['TIMESTAMP'] = pd.to_datetime(df['TIMESTAMP'])
            last_year = df[df['TIMESTAMP'] > datetime.now() - timedelta(days=365)]
